"""Run ONCE to freeze the nonce/arith ("real ICL") dataset:
    ...
    python data/make_nonce_arithmetic_splits.py            # builds configs/nonce_arithmetic_splits.pkl
...
exactly like the Hendel pipeline loads configs/splits.pkl built by make_splits.py.
...
def main():
    ...
    p.add_argument('--n-raw', type=int, default=200, help='candidates generated per task')
    p.add_argument('--n-keep', type=int, default=N_TV_PROMPTS, help='solved prompts kept per task')
    ...
    cache = _ROOT / 'configs' / 'nonce_arithmetic_splits.pkl'

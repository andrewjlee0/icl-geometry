...
class MultiPromptAnalyzer:
    ...
    def _get_token_positions(self, prompt: str, examples: List[ICLExample], query: str) -> Dict[str, List[int]]:
        """Get token positions for inputs, outputs, query, and final position"""
        ...
        def find_text_position(text: str, start_idx: int) -> int:
            ...
            for i in range(start_idx, len(full_tokens) - len(text_tokens) + 1):
                if full_tokens[i:i+len(text_tokens)] == text_tokens:
                    return i + len(text_tokens) - 1  # Return last token of the text
            return start_idx
        ...
        positions = {
            ...
            'output_positions': [],
            'query_position': None,
            'final_position': None  # Position where model generates output
        }
        ...
        # Final position is right after "Output:" in the last line
        positions['final_position'] = len(full_tokens) - 1
    ...
    def analyze_prompt_variation(self, variation: PromptVariation) -> Dict:
        ...
        # Extract activations for all relevant positions
        all_positions = (positions['input_positions'] + 
                        positions['output_positions'] + 
                        [positions['query_position'], positions['final_position']])
    ...
    def _compute_metrics(self, layer_activations: Dict[int, np.ndarray], positions: Dict) -> Dict:
        """Compute comprehensive similarity metrics across layers"""
        
        n_inputs = len(positions['input_positions'])
        n_outputs = len(positions['output_positions'])
        
        metrics = {
            # Original metrics
            'query_to_output_similarity': [],
            'output_cluster_tightness': [],
            'query_norm': [],
            
            # NEW METRICS - Plot 1: Final token similarities
            'final_to_query_similarity': [],
            'final_to_outputs_similarity': [],
            'final_to_inputs_similarity': [],
            
            # NEW METRICS - Plot 2: Cross-position similarities
            'inputs_to_inputs_similarity': [],
            'outputs_to_outputs_similarity': [],
            'query_to_inputs_similarity': [],
            'query_to_outputs_similarity': [],
        }
        
        for layer_idx in sorted(layer_activations.keys()):
            vecs = layer_activations[layer_idx]
            
            if len(vecs) < n_inputs + n_outputs + 2:  # Need all positions
                continue
            
            # Split vectors by position type
            input_vecs = vecs[:n_inputs]
            output_vecs = vecs[n_inputs:n_inputs+n_outputs]
            query_vec = vecs[n_inputs+n_outputs]
            final_vec = vecs[n_inputs+n_outputs+1]
            ...
            # === NEW METRICS - PLOT 1: Final token similarities ===
            # Final to query
            metrics['final_to_query_similarity'].append(cosine_sim(final_vec, query_vec))
            
            # Final to outputs (average)
            final_to_outputs = [cosine_sim(final_vec, ov) for ov in output_vecs]
            metrics['final_to_outputs_similarity'].append(np.mean(final_to_outputs))
            
            # Final to inputs (average)
            final_to_inputs = [cosine_sim(final_vec, iv) for iv in input_vecs]
            metrics['final_to_inputs_similarity'].append(np.mean(final_to_inputs))
            
            # === NEW METRICS - PLOT 2: Cross-position similarities ===
            # Inputs to inputs (pairwise average)
            if len(input_vecs) > 1:
                input_sims = []
                for i in range(len(input_vecs)):
                    for j in range(i+1, len(input_vecs)):
                        input_sims.append(cosine_sim(input_vecs[i], input_vecs[j]))
                metrics['inputs_to_inputs_similarity'].append(np.mean(input_sims))
            else:
                metrics['inputs_to_inputs_similarity'].append(1.0)
            
            # Outputs to outputs (pairwise average)
            if len(output_vecs) > 1:
                output_sims = []
                for i in range(len(output_vecs)):
                    for j in range(i+1, len(output_vecs)):
                        output_sims.append(cosine_sim(output_vecs[i], output_vecs[j]))
                metrics['outputs_to_outputs_similarity'].append(np.mean(output_sims))
            else:
                metrics['outputs_to_outputs_similarity'].append(1.0)
            
            # Query to inputs (average)
            query_to_inputs = [cosine_sim(query_vec, iv) for iv in input_vecs]
            metrics['query_to_inputs_similarity'].append(np.mean(query_to_inputs))
            
            # Query to outputs (average) - this duplicates an existing metric but included for completeness
            query_to_outputs = [cosine_sim(query_vec, ov) for ov in output_vecs]
            metrics['query_to_outputs_similarity'].append(np.mean(query_to_outputs))
    ...
    def plot_final_token_similarities(self, analysis_results: Dict, save_path: str = "final_token_similarities.png"):
        """Plot 1: How similar is the final token to query/outputs/inputs across layers"""
        ...
        if agg['correct']:
            metrics_correct = agg['correct']
            layers = range(len(metrics_correct['final_to_query_similarity']['mean']))
            
            ax1.plot(layers, metrics_correct['final_to_query_similarity']['mean'], 
                    'b-o', label='Final→Query', linewidth=2, markersize=5)
            ax1.plot(layers, metrics_correct['final_to_outputs_similarity']['mean'], 
                    'g-s', label='Final→Outputs', linewidth=2, markersize=5)
            ax1.plot(layers, metrics_correct['final_to_inputs_similarity']['mean'], 
                    'r-^', label='Final→Inputs', linewidth=2, markersize=5)
            
            ax1.set_xlabel('Layer', fontsize=12)
            ax1.set_ylabel('Cosine Similarity', fontsize=12)
            ax1.set_title('Final Token Similarities (Correct Predictions)', fontsize=13)
        ...
        if agg['incorrect']:
            metrics_incorrect = agg['incorrect']
            layers = range(len(metrics_incorrect['final_to_query_similarity']['mean']))
            
            ax2.plot(layers, metrics_incorrect['final_to_query_similarity']['mean'], 
                    'b-o', label='Final→Query', linewidth=2, markersize=5)
            ax2.plot(layers, metrics_incorrect['final_to_outputs_similarity']['mean'], 
                    'g-s', label='Final→Outputs', linewidth=2, markersize=5)
            ax2.plot(layers, metrics_incorrect['final_to_inputs_similarity']['mean'], 
                    'r-^', label='Final→Inputs', linewidth=2, markersize=5)
            
            ax2.set_xlabel('Layer', fontsize=12)
            ax2.set_ylabel('Cosine Similarity', fontsize=12)
            ax2.set_title('Final Token Similarities (Incorrect Predictions)', fontsize=13)
        ...
        print(f"Saved final token similarities plot to {save_path}")
        plt.close()
    
    def plot_cross_position_similarities(self, analysis_results: Dict, save_path: str = "cross_position_similarities.png"):
        """Plot 2: Similarities between different position types across layers"""
        ...
        if agg['correct']:
            metrics_correct = agg['correct']
            layers = range(len(metrics_correct['inputs_to_inputs_similarity']['mean']))
            
            ax1.plot(layers, metrics_correct['inputs_to_inputs_similarity']['mean'], 
                    'c-o', label='Inputs↔Inputs', linewidth=2, markersize=5)
            ax1.plot(layers, metrics_correct['outputs_to_outputs_similarity']['mean'], 
                    'm-s', label='Outputs↔Outputs', linewidth=2, markersize=5)
            ax1.plot(layers, metrics_correct['query_to_inputs_similarity']['mean'], 
                    'y-^', label='Query→Inputs', linewidth=2, markersize=5)
            ax1.plot(layers, metrics_correct['query_to_outputs_similarity']['mean'], 
                    'k-d', label='Query→Outputs', linewidth=2, markersize=5)
            
            ax1.set_xlabel('Layer', fontsize=12)
            ax1.set_ylabel('Cosine Similarity', fontsize=12)
            ax1.set_title('Cross-Position Similarities (Correct Predictions)', fontsize=13)
            ax1.legend(fontsize=10)
            ax1.grid(True, alpha=0.3)
            ax1.set_ylim([0, 1])
        ...
        if agg['incorrect']:
            metrics_incorrect = agg['incorrect']
            layers = range(len(metrics_incorrect['inputs_to_inputs_similarity']['mean']))
            
            ax2.plot(layers, metrics_incorrect['inputs_to_inputs_similarity']['mean'], 
                    'c-o', label='Inputs↔Inputs', linewidth=2, markersize=5)
            ax2.plot(layers, metrics_incorrect['outputs_to_outputs_similarity']['mean'], 
                    'm-s', label='Outputs↔Outputs', linewidth=2, markersize=5)
            ax2.plot(layers, metrics_incorrect['query_to_inputs_similarity']['mean'], 
                    'y-^', label='Query→Inputs', linewidth=2, markersize=5)
            ax2.plot(layers, metrics_incorrect['query_to_outputs_similarity']['mean'], 
                    'k-d', label='Query→Outputs', linewidth=2, markersize=5)
            
            ax2.set_xlabel('Layer', fontsize=12)
            ax2.set_ylabel('Cosine Similarity', fontsize=12)
            ax2.set_title('Cross-Position Similarities (Incorrect Predictions)', fontsize=13)
            ax2.legend(fontsize=10)
            ax2.grid(True, alpha=0.3)
            ax2.set_ylim([0, 1])
        ...
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Saved cross-position similarities plot to {save_path}")
        plt.close()
...
def main():
    ...
    # NEW PLOTS
    analyzer.plot_final_token_similarities(results, f"{prefix}_final_token_sims.png")
    analyzer.plot_cross_position_similarities(results, f"{prefix}_cross_position_sims.png")
    ...
    print(f"  - {prefix}_final_token_sims.png  [NEW]")
    print(f"  - {prefix}_cross_position_sims.png  [NEW]")
